...
| Release sweep | Offline | `uv run --extra dev python -m vtrade.release_verification` | Exact ten migrations, archive boundary, absent legacy paths, and zero active legacy venue references. |
...
| PostgreSQL bootstrap | Real disposable PostgreSQL | `$env:VTRADE_RUN_POSTGRES_INTEGRATION='1'; uv run --extra dev python -m pytest tests/test_postgres_*.py` | Fresh clean-chain apply/rerun, checksum rejection, latest migration, auditable fee-policy, metric, and order-time execution tables, and rollback-only isolation. |
