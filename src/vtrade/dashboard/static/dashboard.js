...
function positionsPanel(positions) {
  const card = element("article", "panel context-card"); card.append(element("h3", "", "Open positions")); const root = element("div", "position-list");
  if (!positions.length) root.append(element("p", "retention-note", "No open positions are recorded."));
  positions.slice(0, 8).forEach((position) => { const row = element("div", "position-row"); const copy = document.createElement("div"); copy.append(element("strong", "", first(position.question, position.market_question, position.market, position.outcome)), element("span", "", `${text(first(position.shares, position.quantity, "—"))} shares · ${text(first(position.valuation_status, "value unavailable"))}`)); row.append(copy, element("strong", classForValue(first(position.unrealized_pnl_micros, position.pnl_micros)), amount(first(position.unrealized_pnl_micros, position.pnl_micros, position.liquidation_value_micros)))); root.append(row); }); card.append(root); return card;
}
...
function renderFreshness(items) {
  ...
  const fragment = document.createDocumentFragment(); items.forEach((item) => { const status = String(first(item.status, item.freshness, "fresh")).toLowerCase(); const row = element("div", "freshness-row"); row.append(element("i", `freshness-dot ${status}`)); const copy = document.createElement("div"); copy.append(element("p", "", first(item.source, item.name, item.dataset)), element("small", "", first(item.cutoff ? `Latest cutoff ${date(item.cutoff)}` : undefined, item.age_seconds !== undefined ? `${item.age_seconds}s old` : undefined, status))); row.append(copy, element("span", `badge ${statusClass(status)}`, status)); fragment.append(row); }); root.replaceChildren(fragment);
}
